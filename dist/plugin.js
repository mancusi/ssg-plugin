    ...
    function patch(fs5) {
      ...
      function patchLchmod(fs6) {
        fs6.lchmod = function(path6, mode, callback) {
          fs6.open(path6, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
            if (err) {
              if (callback)
                callback(err);
              return;
    ...
    function patch(fs6) {
      ...
      function readdir2(path6, options, cb) {
        ...
        function go$readdir(path7, options2, cb2, startTime) {
          return fs$readdir(path7, options2, function(err, files) {
            if (err && (err.code === "EMFILE" || err.code === "ENFILE"))
              enqueue([go$readdir, [path7, options2, cb2], err, startTime || Date.now(), Date.now()]);
            else {
              if (files && files.sort)
                files.sort();
              if (typeof cb2 === "function")
...
var startTimer = () => {
  ...
  return {
    stop: () => {
      ...
      return (0, import_pretty_ms.default)(endTime.valueOf() - startTime.valueOf());
    }
  };
};
...
var addTimingToLog = (prettyPrintedTime, text) => {
  return `${source_default.grey(`[${prettyPrintedTime}]`)} ${text}`;
};
...
var createFeatureJson = async (templateModules, featurePath) => {
  ...
  const featureDir = import_path2.default.dirname(featurePath);
  if (!import_fs_extra2.default.existsSync(featureDir)) {
    import_fs_extra2.default.mkdirSync(featureDir);
  }
  import_fs_extra2.default.writeFileSync(featurePath, JSON.stringify({ features, streams }, null, "  "));
...
var validateModules = (templateModules) => {
  validateUniqueFeatureName(templateModules);
};
var validateUniqueFeatureName = (templateModules) => {
